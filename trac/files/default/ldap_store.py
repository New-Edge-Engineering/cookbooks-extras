...
class LdapAuthStore(Component):
    ...
    def set_password(self, user, password):
        ...
        md5_password = "{MD5}" + base64.encodestring(hashlib.new('md5', password).digest()).rstrip()
    ...
    def check_password(self, user, password, log=True):
        ...
        if m:
            ...
            if (mech == '{MD5}') or (mech == '{md5}'):
                password = mech + base64.encodestring(hashlib.new('md5', password).digest()).rstrip()
            elif (mech == '{CRYPT}') or (mech == '{crypt}'):
                password = mech + crypt.crypt(password, stored[7:])
            elif (mech == '{SSH}') or (mech == '{ssha}'):
                challenge_bytes = base64.decodestring(stored[6:])
                digest = challenge_bytes[:20]
                salt = challenge_bytes[20:]
                hr = hashlib.new('sha', password)
                hr.update(salt)
                password = digest
                stored = hr.digest()
        if stored == password: 
