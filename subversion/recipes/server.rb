...
node[:subversion][:repos].each do |repo|
  execute "svnadmin create repo #{repo}" do
    command "svnadmin create #{node[:subversion][:repo_dir]}/#{repo}"
    creates "#{node[:subversion][:repo_dir]}/#{repo}"
    user "root"
    group node[:apache][:user]
  end  
end
